...
/**
 * <!-- begin-user-doc -->
 * A static utility class that provides operations related to '<em><b>Labeled
 * Drug</b></em>' model objects.
 * <!-- end-user-doc -->
 *
 * <p>
...
public class LabeledDrugOperations extends EntityOperations {
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(labeledDrug))
		{
			if (diagnostics != null)
			{
				diagnostics.add(
						new BasicDiagnostic(Diagnostic.ERROR, CDAValidator.DIAGNOSTIC_SOURCE, CDAValidator.LABELED_DRUG__CLASS_CODE,
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_DETERMINER_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(labeledDrug))
		{
			if (diagnostics != null)
			{
