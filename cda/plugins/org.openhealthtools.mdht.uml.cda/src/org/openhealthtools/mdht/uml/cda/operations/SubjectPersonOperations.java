...
/**
 * <!-- begin-user-doc -->
 * A static utility class that provides operations related to '<em><b>Subject
 * Person</b></em>' model objects.
 * <!-- end-user-doc -->
 *
 * <p>
...
public class SubjectPersonOperations extends EntityOperations {
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(subjectPerson))
		{
			if (diagnostics != null)
			{
    ...
    public static boolean validateDeterminerCode(SubjectPerson subjectPerson, DiagnosticChain diagnostics,
			Map<Object, Object> context)
	{
		if (!EOCL_ENV.createQuery(VALIDATE_DETERMINER_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(subjectPerson))
		{
			if (diagnostics != null)
			{
				diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, CDAValidator.DIAGNOSTIC_SOURCE,
