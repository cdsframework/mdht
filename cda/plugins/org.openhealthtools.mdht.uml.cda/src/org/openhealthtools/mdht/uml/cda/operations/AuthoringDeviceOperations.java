...
/**
 * <!-- begin-user-doc -->
 * A static utility class that provides operations related to '<em><b>Authoring
 * Device</b></em>' model objects.
 * <!-- end-user-doc -->
 *
 * <p>
...
public class AuthoringDeviceOperations extends EntityOperations {
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(authoringDevice))
		{
			if (diagnostics != null)
			{
    ...
    public static boolean validateDeterminerCode(AuthoringDevice authoringDevice, DiagnosticChain diagnostics,
			Map<Object, Object> context)
	{
		if (!EOCL_ENV.createQuery(VALIDATE_DETERMINER_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(authoringDevice))
		{
			if (diagnostics != null)
			{
				diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, CDAValidator.DIAGNOSTIC_SOURCE,
