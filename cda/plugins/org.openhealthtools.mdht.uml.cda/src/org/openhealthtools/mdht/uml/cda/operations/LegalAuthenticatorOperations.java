...
/**
 * <!-- begin-user-doc -->
 * A static utility class that provides operations related to '<em><b>Legal
 * Authenticator</b></em>' model objects.
 * <!-- end-user-doc -->
 *
 * <p>
...
public class LegalAuthenticatorOperations extends ParticipationOperations {
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_TYPE_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(legalAuthenticator))
		{
			if (diagnostics != null)
			{
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_CONTEXT_CONTROL_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(legalAuthenticator))
		{
			if (diagnostics != null)
			{
				diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, CDAValidator.DIAGNOSTIC_SOURCE,
