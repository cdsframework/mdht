...
/**
 * <!-- begin-user-doc -->
 * A static utility class that provides operations related to '<em><b>Structured
...
public class StructuredBodyOperations extends ActOperations {
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(structuredBody))
		{
			if (diagnostics != null)
			{
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(structuredBody))
		{
			if (diagnostics != null)
			{
				diagnostics.add(new BasicDiagnostic(Diagnostic.ERROR, CDAValidator.DIAGNOSTIC_SOURCE,
