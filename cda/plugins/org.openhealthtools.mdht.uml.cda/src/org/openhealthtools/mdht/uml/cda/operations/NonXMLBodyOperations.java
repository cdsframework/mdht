...
/**
 * <!-- begin-user-doc -->
 * A static utility class that provides operations related to '<em><b>Non XML
 * Body</b></em>' model objects.
 * <!-- end-user-doc -->
 *
 * <p>
...
public class NonXMLBodyOperations extends ActOperations {
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_CLASS_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(nonXMLBody))
		{
			if (diagnostics != null)
			{
				diagnostics.add(
						new BasicDiagnostic(Diagnostic.ERROR, CDAValidator.DIAGNOSTIC_SOURCE, CDAValidator.NON_XML_BODY__CLASS_CODE,
	...
	{
		if (!EOCL_ENV.createQuery(VALIDATE_MOOD_CODE__DIAGNOSTIC_CHAIN_MAP__EOCL_INV.get()).check(nonXMLBody))
		{
			if (diagnostics != null)
			{
				diagnostics.add(
						new BasicDiagnostic(Diagnostic.ERROR, CDAValidator.DIAGNOSTIC_SOURCE, CDAValidator.NON_XML_BODY__MOOD_CODE,
								org.eclipse.emf.ecore.plugin.EcorePlugin.INSTANCE.getString("_UI_GenericInvariant_diagnostic",
